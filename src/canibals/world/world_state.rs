...
mod test {
    ...
    fn world_get_son_states_returns_expected_states() {
        ...
        let _actual_son_states = w_s
            .get_son_states()
            .into_iter()
            .map(|ws_result| {
                ws_result.expect("faulty result has been generated by get_son_states() method")
            })
            .collect::<Vec<WorldState>>();
        // for son_state in expected_son_state {
        //     let _expected_state =
        //         son_state.expect("faulty state was added as part of the fixtures");
        // }
    }
}
